  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button
        ...
        className={`group relative flex items-center gap-2 sm:gap-3 px-2 sm:px-4 py-1.5 sm:py-2.5 bg-[#1e1e2e]/80 
      ...
      >
        ...
        {!isMobile && (
          <span className="text-xs sm:text-sm text-gray-200 min-w-[60px] sm:min-w-[80px] text-left group-hover:text-white transition-colors">
            {currentLanguageObj.label}
          </span>
        )}

        <ChevronDownIcon
          className={`size-3 sm:size-4 text-gray-400 transition-all duration-300 group-hover:text-gray-300
            ${isOpen ? "rotate-180" : ""}`}
        />
      </motion.button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full right-0 sm:left-0 mt-2 w-56 sm:w-64 bg-[#1e1e2e]/95 backdrop-blur-xl
              ...
              {Object.values(LANGUAGE_CONFIG).map((lang, index) => {
                ...
                return (
                  ...
                  >
                    <button
                      className={`
                      relative w-full flex items-center gap-3 px-2 sm:px-3 py-2 sm:py-2.5 rounded-lg transition-all duration-200
